...
export var generateSearchQueryTextChangedEpic = function generateSearchQueryTextChangedEpic(schema, doRequest) {
  return function (action$, state$) {
    return action$.pipe(ofType(consts.SEARCH_QUERY_TEXT_CHANGED), map(R.prop('payload')), map(function (payload) {
...
export var generateFetchSearchEntriesEpic = function generateFetchSearchEntriesEpic(schema, doRequest) {
  return function (action$, state$) {
    return action$.pipe(ofType(consts.FETCH_SEARCH_ENTRIES), map(R.prop('payload')), map(function (payload) {
      var query = doRequest.buildQuery(null, 'search');
      var variables = {
      ...
      return doRequest.sendRequest(context.query, context.variables).then(function (_ref) {
        var data = _ref.data,
            error = _ref.error;
        return {
          context: context,
