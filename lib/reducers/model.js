  ...
  return function () {
    ...
    switch (action.type) {
      ...
      case Actions.UPDATE_DELETE_MODEL:
        {
          var _id = R.path(['payload', 'id'], action);

          var _store = _Object$assign({}, R.propOr(getDefaultModelStore(), modelName, state));

          _store.values = R.dissoc(_id, _valuesInstanceProperty(_store));
          _store.order = R.without([_id], _store.order);
          return _Object$assign({}, state, _defineProperty({}, modelName, _store));
        }

      case Actions.REMOVE_INSTANCE:
        {
          var _modelName = R.path(['payload', 'modelName'], action);

          var _id2 = R.path(['payload', 'id'], action);

          return R.assocPath([_modelName, 'values', _id2.toString()], {
            result: null
          }, state);
        }
