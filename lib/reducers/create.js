...
exports.selectCreate = exports.generateCreateReducer = undefined;
...
var stackPush = function stackPush(state, action) {
  ...
  state = R.assoc('stack', R.append({ modelName: R.path(['payload', 'modelName'], action) }, stack), state);
...
var detailCreate = function detailCreate(schema, state, action) {
  ...
  var parentName = (0, _conveyor.getDisplayValue)({
    schema: schema,
    modelName: parentModelName,
    node: node
  });
  var parentId = R.prop('id', node);
  var type = (0, _conveyor.getType)({
    schema: schema,
  ...
  };var prepopulatedField = type.includes('ToMany') ? [fieldData] : fieldData;

  // add prepopulated field to the state, so that upon opening the create form
  // it will already be visible
  state = R.assocPath(['stack', 0, 'fields', targetInverseFieldName], prepopulatedField, enterFormStack(state, action));
...
var CreateValues = function CreateValues(schema, _ref) {
  ...
  }, (0, _conveyor.getFields)(schema, modelName));

  // get index of form stack being saved
  var formStackIndex = R.prop('index', state);

  // get origin page, if create stack entered from detail page
  var originModelName = R.prop('originModelName', state);
  ...
  return Object.assign.apply(Object, [{}].concat(_toConsumableArray(Object.entries(createFields).map(function (_ref2) {
...
var handleStackPop = function handleStackPop(state, action) {
  var stack = R.prop('stack', state);
  stack.pop();
  var newIndex = R.prop('index', state) - 1;
  state = R.assoc('stack', stack, state);
...
var generateCreateReducer = exports.generateCreateReducer = function generateCreateReducer(schema) {
  return function () {
      ...
      case Actions.SAVE:
        {
          ...
          var rawCreateValues = CreateValues(schema, { modelName: modelName, state: state });
...
var selectCreate = exports.selectCreate = function selectCreate(state) {
