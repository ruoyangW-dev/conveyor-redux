...
export const generateTooltipEpic = (schema, doRequest) => (action$, state$) => action$.pipe(
  ...
  mergeMap(context => {
    return doRequest.sendRequest(context.query, context.variables).then(({ data, error }) => ({ context, data, error }))
  }),
  map(({ context, data, error }) => {
    if (error) {
      return Actions.errorLogger({ message: `Error loading ${context.modelName} tooltip` })
    }
